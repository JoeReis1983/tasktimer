...
class TaskTimerApp:
    def __init__(self, root):
        ...
        task_type_combobox = ttk.Combobox(
            root, textvariable=self.task_type, values=task_types
        )
        ...
        tk.Button(root, text="Play", command=self.start_timer).pack(
            side=tk.LEFT
        )
        ...
        tk.Button(root, text="View Log", command=self.view_tasks).pack(
            side=tk.RIGHT
        )
    ...
    def update_timer(self):
        if self.running:
            current_time = (
                int(time.time() - self.start_time) + self.elapsed_time
            )
    ...
    def save_task(self):
        ...
        if name and task_type and duration > 0:
            ...
            messagebox.showwarning(
                "Warning", "Fill in all fields and register a valid duration."
            )
    ...
    def display_results(self, result_day, result_week, result_month):
        day_str = "\n".join(
            [f"{task_type}: {duration}s" for task_type, duration in result_day]
        )
        week_str = "\n".join(
            [f"{task_type}: {duration}s" for task_type, duration in result_week]
        )
        month_str = "\n".join(
            [
                f"{task_type}: {duration}s"
                for task_type, duration in result_month
            ]
        )
