...
class TaskLogic:
    ...
    def save_task(self, name, task_type, duration):
        ...
        ''', (datetime.now().strftime('%Y-%m-%d %H:%M:%S'), name, task_type, duration))
        self.conn.commit()
    ...
    def view_tasks(self):
        ...
        self.cursor.execute('SELECT type, SUM(duration) FROM tasks WHERE date >= ? GROUP BY type', (today,))
        result_day = self.cursor.fetchall()

        self.cursor.execute('SELECT type, SUM(duration) FROM tasks WHERE date >= ? GROUP BY type', (week,))
        result_week = self.cursor.fetchall()

        self.cursor.execute('SELECT type, SUM(duration) FROM tasks WHERE date >= ? GROUP BY type', (month,))
